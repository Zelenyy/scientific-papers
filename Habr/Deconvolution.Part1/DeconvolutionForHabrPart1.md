...
$$
где *апостерионая плотность* $P(\varphi|f)$ определяется по теореме Баейса:
$$
...
#### Установка
Устанвилваем через pip:
```
pip install statreg
```
Или качем исходный [код](https://github.com/mipt-npm/statreg-py).

#### Примеры

В качестве примеров мы рассмотрим как использовать модуль `stareg`, для востановление данных для матричного и интегрального уравения.
...
%matplotlib inline
```
Определяем наш истинный согнал, который мы будем востанавливать.
```python
a = 0
b = 5
# Востанавливаемый сигнал
...
```
В качетсве функционального базиса для дискретизации мы используем кубические сплайны, причем в качетсве дополнительного условия мы укажем, что функция принимает на края нулевые значения.
...
### Julia
Как мы упонминали для дальнейшего развития методики требуется продвинутые методы Монте-Карло моделирования, мы могли бы использовать какой-либо модуль на Python (например, мы проводили работу с PyMC3), но мы помимо прочего усчатвуем в совместном проекте с институтом Макса Планка в Мюнхене. Этот проект называется BAT - , его цель создание фреймфорка с инстурментами для байесовких методов анализа и в первую очередь включается себя иструменты для MCMC. Сейчас команда работает над второй версией фреймворка, которая пишется на Juila, и одна из задач нашей группы это деменстрация возможностей этого фраймоврка напримере статистиеской регуляризации, поэтому мы написали реализацию на Juila. 
...
На этот раз используем другое ядро, будем брать не интегрирущее ступеньку, а свертку с гауссом, которая фактическим наводит некий "блюр" на наши данные
```julia
...
```
![png](output_2_0.png)
Аналогично возьмем базис спалйнов с закрепленными концами:
```julia
...
В нашей статье сделан обзор полезной методики, которую можно адаптировать под многие задача анализа данных (в том  числе машинного обучения), причем получть честную "подгонку" ответа --- получить наиболле рациональное решение уравенние в условияъ неопределенности вызваный ошибками зимерения. Как очень приятный бонус мы получаем значения для ошибки решения, желающие участвовать в развитии или принменять метод статистической регуляризации, могут внести свой вклад в виде кода на Python или Juila или чём-нибудь ещё.

В следующей части мы поговорим о:

* Использовании MCMC
* Разложение Холецкого
* В качетсве практического примера расмотрим применение регуляризации для обработки сигнала с модели орбитального детектора протонов и электронов
