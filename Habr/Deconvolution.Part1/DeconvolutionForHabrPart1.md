...
![График функций](image/first.svg)
...
![Результаты восстановления](image/second.svg)
...
тем, что можно получить аналитическое решение нашей задачи. Поскольку априорная информация и ошибки имеют гауссов видов, то и их произведение имеет гауссов вид и тогда тот страшненький интеграл, который мы писали выше легко берется. Решение и
...
(\vec{\varphi},\Omega\vec{\varphi}))}
$$
Основная проблема в том что этот страшный интеграл, во-первых многомерный, а во-вторых в бесконечных пределах. Причем он сильно многомерный, вектор $\vec{\varphi}$ спокойно может иметь размерность $m = 30-50$, а сеточные методы вычисления интегралов имеют сложность типа $O(n^m)$, поэтому малоприменимы в данном случае. Во взятие многомерных интегралов хорошо работают методы Монте-Карло интегрирования, причем поскольку у нас пределы бесконечные, мы должным использовать метод сущеcтвенной выборки (important sampling), однако тогда нам потребуется подбирать функцию для семплирования. Что бы сделать все более автоматизированным следует использовать [Markov Chain Monte Carlo (MCMC)](https://en.wikipedia.org/wiki/Markov_chain_Monte_Carlo), которое может самостоятельно адаптировать семплирующую функцию под вычисляемый интеграл. Про применение MCMC мы и поговорим в следующей статье.
...
![png](image/julia_8_0.png)

#### Пример на реальных данных
В качестве примера анализа реальных данных мы восстановим спектр рассеяние электронов на водородно-дейтериевой смеси. В опыте измерялся интегральный спектр (то есть число электронов выше опредленной энергии), а нам нужно восстановить дифференциальный спектр. Для этих данных первоначально был восстановлен спектр с помощью фитирования, так что у нас есть основа для проверки корректности нашего алгоритма.
Вот так выглядит исходный интегральный спектр:
![](image/fig01.svg)
А вот так результат восстановления:
![](image/fig02.svg)
Анализ с помощью фита имеет три основных недостатка:

* Параметрическая форма для подгонки не имеет достаточной физической основы, и поэтому результат является субъективным в отношении параметризации.
...
* [Диссертация по применению решуляризации в атмосферной физике](https://drive.google.com/file/d/1VcRmF9h8fhyRq88bpttE5YzX5D-mQpdq/view?usp=sharing)
